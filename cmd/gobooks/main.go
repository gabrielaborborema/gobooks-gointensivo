...
func main() {
	...
	db, err := sql.Open("mysql", "books:books@tcp(35.202.150.173:3306)/books")
	if err != nil {
		panic(err)
	}
	defer db.Close()
	...
	router.HandleFunc("GET /books/search/{bookName}", bookHandlers.SearchBooks)
	router.HandleFunc("GET /books/simulate/{bookIDs}", bookHandlers.SimulateReading)
